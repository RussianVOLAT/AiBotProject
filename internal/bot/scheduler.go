...
func (b *Bot) deliverTo(ctx context.Context, sub domain.Subscription) {
	...
	// библиотеке хранит telegram message_id).
...
// renderAutopushText формирует текст автопуша: полная статистика (цена +
// min/max за день + % за час) по каждой отслеживаемой валюте, а не голая
// цена тем же форматом, что и /rates {currency} (см. formatRateStats
// в handlers.go), чтобы не поддерживать два похожих форматирования.
// Метка времени в конце  не влияет на корректность (это не
// editMessageText, сообщение и так новое), но даёт пользователю видимый
// сигнал "бот жив", даже если курс с прошлого раза не изменился.
func (b *Bot) renderAutopushText(ctx context.Context, now time.Time) string {
	...
	for _, r := range rates {
		stats, err := b.rates.GetRateStats(ctx, r.Currency)
		if err != nil {
			// Не должно случаться (только что получили эту же валюту из
			// GetLatestRates), но на всякий случай не роняем весь пуш
			// из-за одной валюты откатываемся к голой цене для неё.
			b.logger.Error("bot: fetch rate stats for autopush failed", "currency", r.Currency, "err", err)
			fmt.Fprintf(&sb, "%s: %.2f $\n\n", r.Currency, r.PriceUSD)
			continue
		}
		sb.WriteString(formatRateStats(stats))
		sb.WriteString("\n\n")
	}
	fmt.Fprintf(&sb, "обновлено: %s", now.Format("15:04:05"))
	return sb.String()
}
