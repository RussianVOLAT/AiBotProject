...
Основной UI — inline-кнопки поверх этих же команд (см. ADR от 2026-09-13):
`/start` → меню (Курсы / Включить автопуш / Выключить автопуш) → подменю
валют (динамически из БД) или интервалов автопуша (5/15/30/60 мин).
Навигация правит одно сообщение (showScreen), а не создаёт новые — текст
команды по-прежнему работают для тех, кто их набирает.

Автопуш присылает полную статистику (цена + min/max за день + % за час) по
каждой валюте, тем же форматом, что и `/rates {currency}`.
