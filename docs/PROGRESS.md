...
- [x] Тесты: юнит на `domain.Subscription.IsDue`, интеграционный на
      `storage.DueSubscriptions`
## Готово (Этап 2 — добор: кнопки + полная статистика в автопуше)
- [x] Inline-меню (Курсы / Включить автопуш / Выключить автопуш) поверх
      текстовых команд, список валют и интервалов — динамически/кнопками
- [x] Навигация правит одно сообщение (showScreen) вместо создания новых
- [x] Автопуш присылает полную статистику (min/max, % за час), не только цену
- [x] Ручная проверка end-to-end через тестового бота
